...
const HobbyCard: FC<HobbyCardProps> = React.memo(({ hobby, index }) => {
  return (
    <motion.div
      ...
      viewport={{ once: true }}
      className="group relative overflow-hidden rounded-2xl bg-white/8 backdrop-blur-xl border border-white/20 cursor-pointer hover:border-white/40 transition-all duration-400 aspect-video hover:p-0" // aspect-video (16:9 compact)
      role="img"
      tabIndex={0}
